...
class browse(cmd.Cmd):
	...
	def do_model(self, line):
		...
		if self.accesstype == "base":
			if len(line) != 2:
				...
				modeldir = line[1]
			try:
			    um = Personis_a.Access(model=self.umname, modeldir=modeldir, authType="user", auth=self.username + ":" + self.userpassword)
			except:
	...
	def do_export(self, line):
		...
		modeljson = um.export_model(context=self.context, resolver=dict(evidence_filter="all"))
	...
	def do_app(self, line):
		...
		for item in requests:
		    print "App name: " + item
		    description = requests[item]['description']
		    print "Description: " + description 
		    fingerprint = requests[item]['fingerprint']
		    print "Key fingerprint: " + fingerprint 
		    response = raw_input("Authorise this app? (Y/N): ")
		    if response == "Y":
			um.registerapp(app=item, desc=description, fingerprint=fingerprint)
...
#		# TODO: change this to app cert instead of password
#		app_pass = raw_input("Application password: ")
#		# TODO: change this method to take a cert instead of a password
